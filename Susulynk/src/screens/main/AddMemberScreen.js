...
const AddMemberScreen = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await memberService.addMember(groupId, { fullName: form.fullName, phone: form.phone, email: form.email || undefined, role: form.role, notes: form.notes || undefined });
      navigation.goBack();
    } catch (err) { Alert.alert('Error', err.message || 'Could not add member'); }
    finally { setLoading(false); }
  };

  const initials = form.fullName ? form.fullName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() : '?';
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        ...
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.form}>
            <Input label="Full Name *" placeholder="e.g. Akosua Mensah" value={form.fullName} onChangeText={v => update('fullName', v)} error={errors.fullName} />
            <Input label="Phone Number *" placeholder="e.g. 0241234567" value={form.phone} onChangeText={v => update('phone', v)} keyboardType="phone-pad" autoCapitalize="none" error={errors.phone} />
            <Input label="Email (optional)" placeholder="e.g. akosua@email.com" value={form.email} onChangeText={v => update('email', v)} keyboardType="email-address" autoCapitalize="none" error={errors.email} />

            <Text style={styles.roleLabel}>Role</Text>
            <View style={styles.roleRow}>
              {[{ r: 'MEMBER', icon: 'person-outline', label: 'Member' }, { r: 'ADMIN', icon: 'shield-outline', label: 'Admin' }].map(({ r, icon, label }) => (
                <TouchableOpacity key={r} style={[styles.roleBtn, form.role === r && styles.roleBtnActive]} onPress={() => update('role', r)}>
                  <Ionicons name={icon} size={18} color={form.role === r ? Colors.primary : Colors.textSecondary} />
                  <Text style={[styles.roleText, form.role === r && styles.roleTextActive]}>{label}</Text>
                </TouchableOpacity>
              ))}
            </View>

            <Input label="Notes (optional)" placeholder="Any additional notes..." value={form.notes} onChangeText={v => update('notes', v)} multiline numberOfLines={3} />
