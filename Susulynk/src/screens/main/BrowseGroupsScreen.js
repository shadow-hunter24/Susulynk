...
const BrowseGroupsScreen = ({ navigation }) => {
  ...
  const [requesting, setRequesting] = useState({}); // { [groupId]: bool }

  const load = useCallback(async (silent = false) => {
    if (!silent) setLoading(true);
    try {
      const data = await groupService.browseGroups(search);
      setGroups(data);
    } catch (_) {}
  ...
  const handleRequest = async (group) => {
    ...
    try {
      ...
      if (res.member) {
        ...
        // requireApproval = true — request sent
      ...
      setRequesting(r => ({ ...r, [group.id]: false }));
    }
  };
      ...
      <View style={styles.groupCard}>
        ...
        <View style={styles.groupInfo}>
          ...
          <Text style={styles.groupMeta}>
            {memberCount} member{memberCount !== 1 ? 's' : ''} · GHS {item.contributionAmount}/mo
          </Text>
          <Text style={styles.groupAdmin}>Admin: {admin}</Text>
          <View style={styles.tagRow}>
            <View style={styles.tag}>
              <Text style={styles.tagText}>{item.cycleType}</Text>
            </View>
            {item.requireApproval && (
              <View style={[styles.tag, styles.tagWarning]}>
                <Text style={[styles.tagText, { color: Colors.warning }]}>Approval required</Text>
              </View>
            )}
            {!item.requireApproval && (
              <View style={[styles.tag, styles.tagSuccess]}>
                <Text style={[styles.tagText, { color: Colors.success }]}>Open to join</Text>
              </View>
            )}
          </View>
        </View>
        ...
        <TouchableOpacity
          style={[styles.joinBtn, isRequesting && styles.joinBtnDisabled]}
...
const styles = StyleSheet.create({
  ...
  createBtn: { backgroundColor: Colors.primary, paddingHorizontal: Spacing.md, paddingVertical: Spacing.xs + 2, borderRadius: Radius.full },
  ...
  groupIcon: { width: 48, height: 48, borderRadius: 24, alignItems: 'center', justifyContent: 'center', marginRight: Spacing.md, marginTop: 2 },
  ...
  tagWarning: { backgroundColor: Colors.warning + '20' },
  tagSuccess: { backgroundColor: Colors.success + '20' },
  ...
  newGroupBtn: { backgroundColor: Colors.primary, borderRadius: Radius.full, paddingHorizontal: Spacing.xl, paddingVertical: Spacing.sm + 4 },
