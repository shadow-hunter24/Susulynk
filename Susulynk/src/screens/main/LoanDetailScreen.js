...
const LoanDetailScreen = ({ navigation, route }) => {
  ...
  const load = useCallback(async () => {
    try { const data = await loanService.getLoan(groupId, loanId); setLoan(data); }
    catch (_) {}
  ...
  const handleApprove = () => {
    Alert.alert('Approve Loan', 'Approve this loan request?', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Approve', onPress: async () => {
        try { await loanService.updateLoan(groupId, loanId, { status: 'ACTIVE' }); load(); }
  ...
  const handleReject = () => {
    Alert.alert('Decline Loan', 'Decline this loan request?', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Decline', style: 'destructive', onPress: async () => {
        try { await loanService.updateLoan(groupId, loanId, { status: 'REJECTED' }); load(); }
        catch (err) { Alert.alert('Error', err.message); }
      }},
    ]);
  };
  ...
  const handleMarkRepaid = () => {
    Alert.alert('Mark as Fully Repaid', 'Confirm this loan is fully repaid?', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Confirm', onPress: async () => {
        try { await loanService.updateLoan(groupId, loanId, { status: 'REPAID' }); load(); }
        catch (err) { Alert.alert('Error', err.message); }
      }},
    ]);
  };
  ...
  const handleRepayment = async () => {
    ...
    try {
      ...
      setRepayAmount(''); setShowRepayForm(false); load();
      Alert.alert('Recorded', `GHS ${repayAmount} repayment recorded.`);
    } catch (err) { Alert.alert('Error', err.message); }
  ...
  const progress    = Math.min(loan.amountRepaid / loan.amount, 1);
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.container}>
        ...
        <View style={[styles.amountHero, { backgroundColor: heroBg }]}>
          ...
          <View style={styles.amountRow}>
            {[['Repaid', `GHS ${loan.amountRepaid.toLocaleString()}`], ['Outstanding', `GHS ${outstanding.toLocaleString()}`], ['Total Due', `GHS ${loan.totalDue?.toLocaleString()}`]].map(([label, value], i) => (
        ...
        {(loan.status === 'ACTIVE' || loan.status === 'OVERDUE') && (
          <View style={styles.progressCard}>
            <View style={styles.progressHeader}>
              ...
              <Text style={[styles.progressPct, { color: loan.status === 'OVERDUE' ? Colors.error : Colors.success }]}>
            ...
            <Text style={styles.progressSub}>Due: {loan.dueDate ? new Date(loan.dueDate).toLocaleDateString() : '—'}</Text>
          </View>
        )}
        ...
        {showRepayForm && (
          <Card>
            <Text style={styles.repayFormTitle}>{isOwner && !isAdmin ? 'Submit Repayment' : 'Record Repayment'}</Text>
            {isOwner && !isAdmin && (
              <View style={styles.repayInfoBox}>
                <Text style={styles.repayInfoText}>Enter your MoMo reference after sending. The admin will verify it.</Text>
              </View>
            )}
            <Input label="Amount (GHS)" placeholder={`Max: GHS ${outstanding.toLocaleString()}`}
              value={repayAmount} onChangeText={setRepayAmount} keyboardType="decimal-pad" />
        ...
        <View style={styles.actions}>
          {isAdmin && loan.status === 'PENDING' && (
            <>
              ...
              <Button title="Decline Loan" onPress={handleReject} variant="outline" size="md"
                style={{ borderColor: Colors.error, marginBottom: Spacing.sm }} textStyle={{ color: Colors.error }} />
            </>
          )}
          ...
          {canRepay && !showRepayForm && (
            <Button title={isOwner && !isAdmin ? 'Submit Repayment' : 'Record Repayment'} onPress={() => setShowRepayForm(true)} size="lg" style={{ marginBottom: Spacing.sm }} />
          )}
        </View>
...
const makeStyles = (Colors) => StyleSheet.create({
  ...
  infoValue:       { ...Typography.label, color: Colors.textPrimary, flex: 1, textAlign: 'right' },
