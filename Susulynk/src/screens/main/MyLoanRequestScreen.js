...
const MyLoanRequestScreen = ({ navigation }) => {
  ...
  const validate = () => {
    const e = {};
    if (!form.amount || isNaN(principal) || principal < 50)
      e.amount = 'Minimum loan amount is GHS 50';
    if (!form.purpose.trim())
      e.purpose = 'Please state the purpose of the loan';
    setErrors(e);
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await loanService.requestLoan(groupId, {
        amount:   principal,
      ...
      Alert.alert('Error', err.message || 'Could not submit loan request');
    } finally {
      setLoading(false);
    }
  };
      ...
      <SafeAreaView style={styles.safe}>
        <View style={styles.successScreen}>
          ...
          <Text style={styles.successSub}>
            Your GHS {form.amount} loan request has been submitted to the group admin.
          </Text>
          ...
          <View style={styles.form}>
            ...
            <View style={styles.durationRow}>
              {DURATIONS.map(({ label, value }) => (
                ...
                >
                  <Text style={[styles.durationText, form.duration === value && styles.durationTextActive]}>
                    {label}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            <Input
              ...
              placeholder="Any collateral or extra info for the admin..."
              value={form.notes}
            ...
            <View style={styles.infoBox}>
              <Text style={styles.infoText}>
                📋 Your loan request will be reviewed by the group admin. Interest rate is set by the group ({interestRate}%).
              </Text>
            </View>
